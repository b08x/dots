...
SENSITIVE_DCONF_PATHS = [
    b"[org/gnome/keyring]",
    b"[org/gnome/shell/keybindings]",
    b"password=",
    b"passwd=",
    b"secret=",
    b"token=",
...
def filter_sensitive_data(dconf_dump: bytes) -> bytes:
    lines = dconf_dump.split(b"\n")
    filtered_lines = []
    in_sensitive_block = False

    for line in lines:
        if line.startswith(b"["):
            in_sensitive_block = any(
                sensitive in line
                for sensitive in SENSITIVE_DCONF_PATHS
                if b"[" in sensitive
            )
        ...
        if any(
            sensitive in line
            for sensitive in SENSITIVE_DCONF_PATHS
            if b"[" not in sensitive
        ):
            filtered_lines.append(b"# REDACTED: sensitive data removed")
            continue
...
def export_settings():
    export_firefox_settings = input(
        ">>>> Do you want to export firefox settings? The filesize will be WAY bigger. [y/N] "
    ).lower()

    print(">>>> Starting to export settings...")
    dconf_settings = subprocess.run(
        ["dconf", "dump", "/"], capture_output=True, check=False
    ).stdout

    # Filter out sensitive data patterns
    clean_dconf_settings = filter_sensitive_data(dconf_settings)
    clean_dconf_settings = clean_dconf_settings.replace(
        getpass.getuser().encode("utf8"), b"%%USER%%"
    )
    ...
    print(
        ">>>> Sensitive dconf settings (passwords, tokens, secrets) have been filtered out."
    )
    if export_firefox_settings == "y":
        print(
            ">>>> WARNING: Firefox settings WILL contain personal and sensitive data."
        )
        print(">>>> DO NOT share this tar file with anyone. Store it securely.")
    else:
        print(
            ">>>> This export should be safe to share, but review before distributing."
        )
    print(
        "---------------------------------------------------------------------------\n"
    )
...
def import_settings():
    ...
    subprocess.run(
        ["tar", "-xzf", TAR_FILENAME, "-C", os.path.expanduser("~")], check=True
    )
