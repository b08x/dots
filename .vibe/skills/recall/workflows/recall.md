...
Load context from vault memory - supports single-platform (Claude Code only) and multi-platform recall with GitHub/restic integration.

## Step 1: Classify Query

Parse the user's input after `/recall` and classify:

- **Multi-platform** - mentions multiple platforms, "across platforms", "all AI tools", or contains "platform:", "github:", "backup:"
  -> Go to Step 2D
...
## Step 2D: Multi-Platform Recall

Run the multi-platform extraction script:

```bash
python3 .claude/skills/recall/scripts/multi-platform-extract.py [options] [query]
```

**Basic usage patterns:**
```bash
# All platforms, yesterday
python3 multi-platform-extract.py yesterday

# All platforms, with GitHub integration
python3 multi-platform-extract.py --github owner/repo last week

# All platforms, with backup analysis
python3 multi-platform-extract.py --backup ~/Workspace this week

# Specific platform only
python3 multi-platform-extract.py --platform hermes auth work

# Full integration (platforms + GitHub + backups)
python3 multi-platform-extract.py --github myrepo --backup ~/Workspace authentication refactor
```

**Command construction from user query:**
- Look for `platform:NAME` -> add `--platform NAME`
- Look for `github:REPO` -> add `--github REPO`
- Look for `backup:PATH` -> add `--backup PATH`
- Date expressions (yesterday, last week, etc.) passed as positional args
- Topic keywords passed as positional args
...
Present the summary table, key correlations, and the One Thing action to the user.

## Notes

- **Single-platform queries** go through `recall-day.py` (native JSONL, no QMD needed)
- **Multi-platform queries** go through `multi-platform-extract.py` (cross-platform aggregation)
