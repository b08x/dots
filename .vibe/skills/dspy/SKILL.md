...
All providers use `dspy.LM()` with a `provider/model` string:

```python
import dspy

# Anthropic Claude
lm = dspy.LM("anthropic/claude-sonnet-4-5-20250929", api_key="your-api-key", max_tokens=1000, temperature=0.7)
dspy.configure(lm=lm)

# OpenAI
lm = dspy.LM("openai/gpt-4o", api_key="your-api-key", max_tokens=1000)
dspy.configure(lm=lm)

# Local Models (Ollama) — use ollama/ prefix
...
strong_lm = dspy.LM("anthropic/claude-sonnet-4-5-20250929")
...
```python
# Inspect LM call history (enabled by default)
result = qa(question="...")

# Access history via the LM object
for entry in lm.history:
    print(f"Prompt: {entry['messages']}")
    print(f"Response: {entry['outputs']}")
```
